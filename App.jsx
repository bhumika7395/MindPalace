...
const Logo = ({size=28,color=C.accent}) => (
  <svg width={size} height={size} viewBox="0 0 64 64" fill="none">
    <path d="M32 4C24 4 18 10 18 16c0 3 1 5 3 7-4 2-7 6-7 11 0 4 2 7 5 9-2 2-3 5-3 8 0 6 5 11 11 11h11c6 0 11-5 11-11 0-3-1-6-3-8 3-2 5-5 5-9 0-5-3-9-7-11 2-2 3-4 3-7 0-6-6-12-14-12z" fill={color} opacity="0.15"/>
    ...
    <circle cx="32" cy="28" r="3" fill={color} opacity="0.3"/><circle cx="25" cy="34" r="2" fill={color} opacity="0.25"/><circle cx="39" cy="34" r="2" fill={color} opacity="0.25"/>
  </svg>
);
    ...
    <div style={{minHeight:"100vh",background:C.deep,color:C.text,fontFamily:"'DM Sans',sans-serif"}}><style>{css}</style>
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"56px 24px",position:"relative"}}>
        <div style={{animation:"fu .6s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:40}}><Logo size={36}/><span style={{fontSize:15,fontWeight:600,letterSpacing:2.5,textTransform:"uppercase",color:C.accent}}>MindPalace</span></div>
    ...
    <div style={{height:"100vh",background:C.deep,color:C.text,fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column",overflow:"hidden"}} onMouseUp={()=>{setDragging(null);setDrawing(false);}} onMouseLeave={()=>{setDragging(null);setDrawing(false);}}>
      ...
      <div style={{height:44,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 12px",flexShrink:0,background:C.bg,zIndex:50}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          <button onClick={e=>{e.stopPropagation();setChatOpen(p=>!p);setSc(null);}} style={{background:chatOpen?C.accentBg:"transparent",border:`2px solid ${chatOpen?C.accent:C.border}`,borderRadius:10,padding:4,cursor:"pointer",display:"flex",width:36,height:36,alignItems:"center",justifyContent:"center",zIndex:55}}><Logo size={22} color={chatOpen?C.accent:C.muted}/></button>
        </div>
      </div>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",position:"relative"}}>
        ...
        <div ref={boardRef} onClick={handleBoardClick} onContextMenu={handleBoardContext} onDoubleClick={handleBoardDblClick} onMouseDown={handlePenDown} onMouseMove={e=>{onDrag(e);handlePenMove(e);}}
          ...
          {!cards.length&&!showOnboard&&!sc&&<div style={{position:"absolute",top:"35%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",opacity:.3}}><Logo size={48}/><p style={{fontSize:13,color:C.soft,maxWidth:240,lineHeight:1.5,marginTop:10}}>Right-click to chat with AI. Use toolbar to add cards.</p></div>}
          ...
          {showOnboard&&<div style={{position:"absolute",inset:0,background:"rgba(10,8,16,.7)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",animation:"fi .3s"}}>
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"28px 32px",maxWidth:420,boxShadow:"0 20px 60px rgba(0,0,0,.5)",animation:"pop .25s"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><Logo size={36}/><span style={{fontSize:16,fontWeight:600,color:C.accent}}>Welcome to MindPalace</span></div>
          ...
          {sc&&<div data-ui="true" style={{position:"absolute",left:Math.min(sc.x,600),top:sc.y,width:300,background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,boxShadow:"0 16px 48px rgba(0,0,0,.5),0 0 30px rgba(180,160,255,.05)",zIndex:100,animation:"pop .2s",display:"flex",flexDirection:"column",maxHeight:340}}>
            <div style={{padding:"8px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:6}}><Logo size={16}/><span style={{fontSize:11,fontWeight:500,color:C.accent}}>Smart Cursor</span></div><button onClick={()=>setSc(null)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13}}>x</button></div>
        ...
        {chatOpen&&<div style={{width:360,background:C.bg,borderLeft:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0,animation:"sR .2s",zIndex:45}}>
          <div style={{padding:"10px 14px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:8}}><Logo size={20}/><span style={{fontSize:13,fontWeight:500,color:C.accent}}>MindPalace Chat</span></div><button onClick={()=>setChatOpen(false)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:15}}>x</button></div>
